...
def test_classification_cascade(limit=1000):
    ...
    print(f"Results:")
    print(f"  Tier 0 (Rules):    {tier0_filtered:4d} / {total} ({tier0_pct:5.1f}%) filtered")
    print(f"  Tier 1 (Features): {tier1_filtered:4d} / {total} ({tier1_pct:5.1f}%) filtered")
    print(f"  Tier 2 (LLM):      {tier2_required:4d} / {total} ({tier2_pct:5.1f}%) required")
    print(f"\n{'='*60}")
