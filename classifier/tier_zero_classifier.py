...
def tier_zero_classifier(event: dict) -> TierZeroClassification | None:
    ...
    if (
        ...
        and window_context["process_name"] in UNINTERESTING_PROCESSES
    ):
        return TierZeroClassification(
    ...
    if (
        ...
        and window_context["current_window_title"]
        == prev_context["current_window_title"]
    ):
        return TierZeroClassification(
            verdict="not_interesting", score=0, reason="Context change to same window"
        )
