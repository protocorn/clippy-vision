...
def predict(text: str, model, tokenizer, device, threshold: float):
    ...
    secondary   = [
        ID2LABEL[i] for i, p in enumerate(probs)
...
def main():
    parser = argparse.ArgumentParser(description="Test the fine-tuned router classifier")
    ...
    if args.query:
        primary, secondary, probs = predict(args.query, model, tokenizer, device, args.threshold)
