...
BATCH_SIZE          = 32
SEED                = 42
SECONDARY_THRESHOLD = 0.20  # softmax score for a non-primary label to count as secondary hint
...
def decode_prediction(logits: torch.Tensor, threshold: float = SECONDARY_THRESHOLD):
    ...
    secondary   = [
        ID2LABEL[i] for i, p in enumerate(probs.tolist())
...
def train(args):
    ...
    for cat in CATEGORIES:
        print(f"  {cat:<22} primary={dist.get(cat, 0):>4}  as-secondary={sec_dist.get(cat, 0):>4}")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        print(f"[epoch {epoch}/{args.epochs}]  loss={avg_loss:.4f}  train_acc={train_acc:.3f}  eval_acc={eval_acc:.3f}")
...
def eval_only(args):
    ...
    print(f"[eval] {len(eval_ex)} held-out examples  (secondary threshold={args.threshold})")
...
def main():
    ...
    parser.add_argument("--threshold", type=float, default=SECONDARY_THRESHOLD,
