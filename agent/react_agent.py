...
def _build_system_prompt(
    ...
    history = (
        _build_conversation_history(conversation_id, q_vec)
        if user_message
...
def _compress_old_tool_messages(messages: list[dict], keep_last: int = 1) -> None:
    ...
    for i in tool_indices[:-keep_last]:
        ...
        messages[i]["content"] = (
            f"[prior tool result: ~{row_count} rows, already processed]"
        )
...
def _prepare_turn(user_message: str, conversation_id: str):
    ...
    if decision:
        print(
            f"[router] {decision.primary} (conf={confidence:.2f}) secondary={decision.secondary}"
        )
...
def _finalize_answer(
    user_message: str, conversation_id: str, thinking: str, answer: str
) -> str:
