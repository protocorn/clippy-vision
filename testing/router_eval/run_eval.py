...
def prefetch_analysis(records: list[dict]) -> str:
    ...
    n = len(records)
    lines.append(
        f"Prefetch gate fired: {len(fired)}/{n} ({len(fired) / n:.0%} of queries)"
    )
    if fired:
        correct = sum(1 for r in fired if r["gold"] == r["pred"])
        group_ok = sum(
            1 for r in fired if ACTION_GROUP[r["gold"]] == ACTION_GROUP[r["pred"]]
        )
        lines.append(
            f"  precision when fired (exact label):  {correct / len(fired):.1%}"
        )
        lines.append(
            f"  precision when fired (action group): {group_ok / len(fired):.1%}"
        )
    ...
    lines.append(
        f"  prefetchable queries NOT fired (missed speedups): {len(missed)}/{n}"
    )
    ...
    for cat, thr in PREFETCH_THRESHOLDS.items():
        ...
        lines.append(
            f"    {cat:<18} thr={thr:.2f}  fired={len(cat_fired):>3}  precision={ok / len(cat_fired):.1%}"
        )
    return "\n".join(lines)
...
def threshold_sweep(records: list[dict]) -> str:
    ...
    for thr in [0.0, 0.3, 0.4, 0.5, 0.6, 0.7, 0.8, 0.9]:
        ...
        gprec = sum(
            ACTION_GROUP[r["gold"]] == ACTION_GROUP[r["pred"]] for r in sub
        ) / len(sub)
...
def main():
    ...
    parser.add_argument(
        "--with-llm", action="store_true", help="also run qwen3:8b router (slow)"
    )
    ...
    for res in results:
        ...
        out_lines.append(
            f"Action-group accuracy: {res['group_accuracy']:.1%}   "
            "(pred maps to same data source as gold)"
        )
        out_lines.append(
            f"Latency ms: mean={res['latency']['mean']:.1f}  "
    ...
    for res in results:
        with (RESULTS / f"records_{res['name']}.jsonl").open(
            "w", encoding="utf-8"
        ) as f:
