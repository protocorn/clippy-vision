...
def command_parts(config: dict[str, str]) -> list[str]:
    ...
    if not located:
        raise FileNotFoundError(f"{executable} is not installed or is not on PATH.")
    parts[0] = located
    return parts


def _authentication_status(provider: str, parts: list[str]) -> tuple[bool, str]:
    args = ["login", "status"] if provider == "codex_cli" else ["auth", "status"]
    try:
...
def provider_status(config: dict[str, str]) -> dict[str, Any]:
    ...
    try:
        ...
        authenticated, auth_detail = _authentication_status(provider, parts)
        return {
            "ok": authenticated,
            "provider": provider,
            ...
            "capabilities": {
                "chat": {
                    "model": config.get("chat_model", "default"),
                    "available": True,
                },
                "vision": {
                    "model": config.get("vision_model", "default"),
                    "available": True,
                },
            },
            "message": (
                f"{metadata['label']} is signed in."
                if authenticated
                else f"{metadata['label']} CLI is installed but not signed in."
            ),
            **({"error": auth_detail or "Sign in is required."} if not authenticated else {}),
        }
...
def _prompt_from_messages(
    ...
    if provider == "codex_cli":
        sections.append(
            "Use the supplied activity context as your only Clippy context."
        )
    if image_paths:
        sections.append(
    ...
    if tools:
        sections.append(
            "Available Clippy tools:\n"
            + json.dumps(tools, ensure_ascii=False)
            + "\nIf a tool is needed, return only a JSON object with a tool_calls array "
            "using the OpenAI function-call shape. Do not run the tool yourself."
        )
    return "\n\n".join(sections)
...
def _tool_calls_from_text(content: str) -> list[dict[str, Any]]:
    ...
    for index, call in enumerate(calls):
        ...
        normalized.append(
            {
                "id": call.get("id") or f"cli_call_{index}",
                "type": "function",
                "function": {
    ...
    *,
    ...
    with _CLI_LOCK:
        with tempfile.TemporaryDirectory(prefix="clippy-cli-") as temporary:
            ...
            prompt = _prompt_from_messages(messages, image_paths, schema, provider, tools)
            ...
            try:
                ...
                raise RuntimeError(
                    f"Could not start {provider_label(provider)}: {exc}"
                ) from exc
            if process.returncode != 0:
                detail = (stderr or stdout or "CLI returned an error").strip()
                raise RuntimeError(
                    f"{provider_label(provider)} failed: {detail[-1600:]}"
                )
            content = _extract_output(provider, stdout)
            if not content:
                detail = (stderr or "CLI returned no response").strip()
                raise RuntimeError(
                    f"{provider_label(provider)} returned no response: {detail[-800:]}"
                )
            message = {"role": "assistant", "content": content}
            tool_calls = _tool_calls_from_text(content) if tools else []
            if tool_calls:
                message["content"] = ""
